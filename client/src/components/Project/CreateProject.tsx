...
const CreateProjectModal = ({ open, onClose, onCreated }: Props) => {
  ...
  const deanRef = useRef<HTMLDivElement>(null)

  // 🔹 Fetch all HOD & Dean emails ONCE when modal opens
  useEffect(() => {
    if (!open) return
    ...
    const fetchAll = async () => {
      try {
        const [hodRes, deanRes] = await Promise.all([
          ...
          fetch(
            `${apiLink}/api/admin/department-authority/suggest?role=DEAN`,
            { credentials: "include" }
          ),
        ])
        ...
        if (hodData.success) setAllHods(hodData.emails)
        if (deanData.success) setAllDeans(deanData.emails)
      } catch (err) {
        console.error("Failed to fetch HOD/Dean lists", err)
      }
    }
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (
        deanRef.current &&
        !deanRef.current.contains(e.target as Node)
      ) {
        ...
        <div className="relative z-10 w-full max-w-xl mx-4">
          <div className="bg-white/90 backdrop-blur rounded-[28px] shadow-[0_25px_70px_rgba(0,0,0,0.25)] overflow-hidden">
            ...
            <div className="px-8 py-6 space-y-4">
              ...
              <div className="relative" ref={deanRef}>
                <input
                  placeholder="Dean Email"
                  className="w-full border border-slate-300 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-blue-500 outline-none"
                  value={deanEmail}
                  onChange={e => setDeanEmail(e.target.value)}
                  onFocus={() => setShowDeanDropdown(true)}
                />

                {showDeanDropdown && allDeans.length > 0 && (
                  <div className="absolute z-20 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-md max-h-40 overflow-y-auto">
                    {allDeans.map(email => (
                      <div
                        key={email}
